...
<Meta 
    ...
    argTypes={{
        ...
        text: { control: {type: 'text'}, description: 'Text im Button, optional' },
    }}
/>
...
<Template let:args>
    <ProseDecorator>
        <Button {...args} />
        <code><pre>{`<Button`}{#each Object.keys(args) as key}{` ${key}="${args[key]}"`}{/each}{`></Button>`}</pre></code>
    </ProseDecorator>
</Template>

<Story name="All Props" args={{type: 'default', link: '//google.de/sample.hex', download: 'Beispiel.hex', color: 'grau', rounded: 'full', text: 'Text'}} />
...
<Story name="Link Arten">
    <Button link="./">Interner Link</Button>

    <Button link="#Button">Seiteninterner Link</Button>

    <Button link="//google.de">Externer Link</Button>
